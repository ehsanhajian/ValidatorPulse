...
def resolve_reward_token(
    *,
    ...
    if chain_key == "bsc":
        return TokenInfo(
            symbol=(symbol_override or "BNB").upper(),
    ...
    if chain_key == "avalanche":
        return TokenInfo(
            symbol=(symbol_override or "AVAX").upper(),
            decimals=decimals_override if decimals_override is not None else 9,
            base_unit="nAVAX",
        )
    ...
    if chain_key == "mina":
        return TokenInfo(
            symbol=(symbol_override or "MINA").upper(),
            decimals=decimals_override if decimals_override is not None else 9,
            base_unit="nanomina",
        )
    ...
    if chain_key == "ton":
        return TokenInfo(
            symbol=(symbol_override or "GRAM").upper(),
            decimals=decimals_override if decimals_override is not None else 9,
            base_unit="nanoton",
        )
